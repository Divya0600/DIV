...
logspath = '//172.11.0.10/rotorenderlogs/logs/silhoute/'
forIdpath = '//172.11.0.10/rotorenderlogs/logs/nuke/'

class Silhouette_Renderer_Class(object):
    ...
    def launchdummy(self,filename,host,*args):
        ...
        if self.affinity=='None':
            mkfulcmds= coliationpath+'control.exe -t "'+ filename +'" -c "echo" '+host+' add > "'+forIdpath.replace('/','\\')+'idno.txt"'
        else:
            mkfulcmds= coliationpath+'control.exe -t "'+ filename +'" -a "'+self.affinity+'" -c "echo" '+host+' add > "'+forIdpath.replace('/','\\')+'idno.txt"'
        os.system(mkfulcmds)

    def readid(self,*args):
        self.id = ""
        submit=open(forIdpath.replace('/','\\')+'idno.txt', 'r')
        self.id=submit.read()
        submit.close            
    ...
    def mkcd(self,filename,startframe,batch,id,host,*args):
        ...
        eid=re.split('\W+', id)
        eid=eid[0]
        mkcmd='"'+self.executablepath+'" -range '+str(startframe)+'-'+str(endframe)+' "'+self.mainpath+'"'
        ...
        sub_write.close()
        if self.affinity=='None':
            mkfulcmd= coliationpath+'control -t "frames '+ startframe + '-' + endframe +'" -c "'+ batfile +'" -P '+ eid +' '+host+' add >> "'+forIdpath+'launchids.txt"'
        else:
            mkfulcmd= coliationpath+'control -t "frames '+ startframe + '-' + endframe +'" -a "'+self.affinity+'" -c "'+ batfile +'" -P '+ eid +' '+host+' add >> "'+forIdpath+'launchids.txt"'
        os.system(mkfulcmd)
